export default class Carousel {
        ...
        this.central = carousel.central;                                    // центрировать ли элементы карусели
    ...
    initPosition() {
        ...
        if (this.central) {
            leftCentral = central - Math.round(this.itemWidth / 2); // Центрируем centralItem
        } 

        // определяем высоту обертки карусели по максимальной высоте элемента
        let maxHeight = 0;
        Array.from(this.carouselContainer.children).forEach(item => {
    ...
    // addOrUpdateRule(this.container, 'height', '500px');
